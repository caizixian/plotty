...
class LBOBlock(Block):
    ...
    def decode(self, param_string, cache_key):
        ...
        # At least three parts - flagword, type, groups (possibly empty)
        if len(parts) != 3:
            raise PipelineError("LBO block invalid: incorrect number of parts")
        ...
        def sort_keys(l):
            ...
            def isFloat(x):
              try:
                ...
                return False
            is_numeric = all(map(isFloat, l))
            if is_numeric:
              return l.sort(key=float)
            l.sort(key=lambda x: None if isinstance(x, ScenarioValue) else str(x))
            l.sort(key=lambda x: None if isinstance(x, ScenarioValue) or not isFloat(x) else float(x))
